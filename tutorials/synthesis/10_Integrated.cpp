...
gam::ArrayPow2<float> tbSaw(2048), tbSqr(2048), tbImp(2048), tbSin(2048),
    tbPls(2048), tbDin(2048), tb__1(2048), tb__2(2048), tb__3(2048),
    tb__4(2048);
...
public:
  ...
  void onProcess(Graphics &g) override {
    ...
    g.pushMatrix();
    g.translate(amplitude + frequency / 2000, amplitude / 2 + frequency / 2000,
                -4);
    g.scale(0.1, 0.1, 0.1);
...
public:
  ...
  void onProcess(AudioIOData &io) override {
    updateFromParameters();
    mVib.freq(mVibEnv());
    float carBaseFreq = getInternalParameterValue("frequency") *
                        getInternalParameterValue("carMul");
    float modScale = getInternalParameterValue("frequency") *
                     getInternalParameterValue("modMul");
    float amp = getInternalParameterValue("amplitude");
    while (io()) {
      mVib.freq(mVibEnv());
  ...
  void onTriggerOn() override {
    ...
    float modFreq = getInternalParameterValue("frequency") *
                    getInternalParameterValue("modMul");
    mod.freq(modFreq);
...
public:
  ...
  virtual void init() {
    ...
    mAmpEnv.levels(0, 0.3, 0.3,
                   0); // These tables are not normalized, so scale to 0.3
    //        mAmpEnv.sustainPoint(1); // Make point 2 sustain until a release
    //        is issued

    mTrmEnv.curve(0);
    mTrmEnv.levels(0, 1, 1, 0);
    //        mTrmEnv.sustainPoint(1); // Make point 2 sustain until a release
    //        is issued
  ...
  virtual void onProcess(AudioIOData &io) override {
    ...
    while (io()) {
      ...
      float trmAmp =
          (mTrm() * 0.5 + 0.5) * trmDepth + (1 - trmDepth); // Corrected
  ...
  void updateFromParameters() {
    ...
    mTrmEnv.levels(
        getInternalParameterValue("trm1"), getInternalParameterValue("trm2"),
        getInternalParameterValue("trm2"), getInternalParameterValue("trm1"));
...
public:
  ...
  virtual void onProcess(AudioIOData &io) override {
    ...
    while (io()) {
      ...
      s1 = s1 * (1 - amAmt) +
           (s1 * mAM()) * amAmt; // mix modulated and non-modulated
  ...
  virtual void onTriggerOn() override {
    ...
    mAMEnv.levels(
        getInternalParameterValue("am1"), getInternalParameterValue("am2"),
        getInternalParameterValue("am2"), getInternalParameterValue("am1"));
...
public:
  ...
  float operator()() { return (*this)(noise() * env()); }
  float operator()(float in) { return delay(fil(delay() + in)); }
...
public:
  ...
  void fillTime(float from, float to, float minattackStri, float minattackLow,
                float minattackUp, float maxattackStri, float maxattackLow,
                float maxattackUp, float minFreq, float maxFreq) {
    while (from <= to) {
      float nextAtt =
          gam::rnd::uni((minattackStri + minattackLow + minattackUp),
                        (maxattackStri + maxattackLow + maxattackUp));
      auto *voice = synthManager.synth().getVoice<AddSyn>();
      voice->setTriggerParams(
          std::vector<float>{0.03, 440,  0.5,  0.0001, 3.8, 0.3,  0.4,  0.0001,
                             6.0,  0.99, 0.3,  0.0001, 6.0, 0.9,  2,    3,
                             4.07, 0.56, 0.92, 1.19,   1.7, 2.75, 3.36, 0.0});
      voice->setInternalParameterValue("attackStr", nextAtt);
      voice->setInternalParameterValue("frequency",
                                       gam::rnd::uni(minFreq, maxFreq));
      synthManager.synthSequencer().addVoiceFromNow(voice, from, 0.2);
      std::cout << "old from " << from << " plus nextnextAtt " << nextAtt
                << std::endl;
      from += nextAtt;
    }
  }

  void fillTimeWith12TET(float from, float to, float minattackStri,
                         float minattackLow, float minattackUp,
                         float maxattackStri, float maxattackLow,
                         float maxattackUp) {
    while (from <= to) {
      ...
      synthManager.synthSequencer().addVoiceFromNow(voice, from, 0.2);
      std::cout << "12 old from " << from << " plus nextAtt " << nextAtt
                << std::endl;
      from += nextAtt;
    }
  }
