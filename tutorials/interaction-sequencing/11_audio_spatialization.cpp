...
class MyVoice : public SynthVoice {
 public:
  ...
  virtual bool setTriggerParams(float *pFields, int numFields) override {
    if (numFields != 6) {  // Sanity check to make sure we are getting the right
                           // number of p-fields
      return false;
    }
  ...
  virtual int getTriggerParams(float *pFields, int maxParams = -1) override {
    if (maxParams < 6) {
      return 0;
    }  // Sanity check
    // For getParamFields, we will copy the internal parameters into the pointer
    // recieved.
    pFields[0] = mPose.x();
    pFields[1] = mPose.y();
    pFields[2] = mSize;
