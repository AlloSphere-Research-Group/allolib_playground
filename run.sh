...
set -x
"${CMAKE_BINARY}" -G "${GENERATOR}" -DCMAKE_POLICY_VERSION_MINIMUM=3.5 -Wno-deprecated -DRTAUDIO_API_JACK=OFF -DRTMIDI_API_JACK=OFF -DCMAKE_BUILD_TYPE=${BUILD_TYPE} -DAL_APP_FILE=../../../${APP_FILE} -DAL_VERBOSE_OUTPUT=${VERBOSE_FLAG} ${VERBOSE_MAKEFILE} "${INITIALDIR}" > cmake_log.txt
set +x
